...
function Entity.new(x, y)
	...
	self.stats = {
		movespeed = {100, "Movespeed", 20, 0},
		maxHealth = {1, "Max Health", 10, 0},
    ...
    if Game.stateManager.currentID == "explore" then
        ...
        if self.AI == "wander" then
            ...
            if dist > 2 then
                ...
                if roll == 1 then
                    self.AI = "standing"
                    self.standTimer = love.math.random() * 2 + 1
                end
            end

        elseif self.AI == "standing" then
            self.standTimer = self.standTimer - dt
            if self.standTimer <= 0 then
				local roll = love.math.random(2)
				if roll == 1 then
                	self.AI = "wander"
